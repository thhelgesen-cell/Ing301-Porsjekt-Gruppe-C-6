...
class Sensor(Device):
    ...
    def get_current(self) -> Measurement:
        return self.current

    def set_current(self, measurement) -> Measurement:
...
class Room:
    ...
    def __init__(self, floor: Floor, room_size: float, rid:int, room_name: Optional[str]):
        self.floor = floor
...
class SmartHouse:
    ...
    def register_room(self, floor: Floor, room_size: float, room_name: Optional[str] = None) -> Room:
        ...
        """
        room = Room(floor, room_size, self.get_next_rid(), room_name)
